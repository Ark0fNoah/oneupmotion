	...
	<div class="about-story__inner section__inner">
		...
		<div>
			...
			<ul class="about-story__points">
				<?php foreach ( $points as $point ) : ?><li><?php oum_section_icon( $point, 'about-story__point-icon' ); ?><strong><?php echo esc_html( $point['title'] ?? '' ); ?></strong><?php if ( ! empty( $point['text'] ) ) : ?><em><?php echo esc_html( $point['text'] ); ?></em><?php endif; ?></li><?php endforeach; ?>
			</ul>
		</div>
	</div>
</section>
