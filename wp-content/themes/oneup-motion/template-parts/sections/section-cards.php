...
?>
<section class="oum-section oum-section-cards section">
		...
		<div class="oum-card-grid oum-card-grid--<?php echo esc_attr( $layout ); ?>">
			<?php foreach ( $cards as $card ) : ?>
				<article class="tool-card">
					<?php if ( ! empty( $card['icon'] ) ) : ?><div class="tool-card__icon"><?php echo esc_html( $card['icon'] ); ?></div><?php endif; ?>
					<h3><?php echo esc_html( $card['title'] ?? '' ); ?></h3>
					<p><?php echo esc_html( $card['text'] ?? '' ); ?></p>
					<?php if ( ! empty( $card['button_label'] ) && ! empty( $card['button_url'] ) ) : ?><a class="tool-card__button" href="<?php echo esc_url( $card['button_url'] ); ?>"><?php echo esc_html( $card['button_label'] ); ?></a><?php endif; ?>
				</article>
			<?php endforeach; ?>
		</div>
	</div>
